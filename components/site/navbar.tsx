...
export function Navbar() {
  ...
  /*
   * Добавляем Автокредит в меню.
   * Если он уже есть в navLinks — второй раз не добавляем.
   */
  const menuLinks = navLinks.some(
    (link) => link.label.toLowerCase() === "автокредит"
  )
    ? navLinks
    : [
        ...navLinks.slice(0, 1),
   ...
   * Также блокируем прокрутку страницы,
  ...
  return (
    <>
      ...
      <a
        href="#inventory"
        className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100] focus:rounded-xl focus:bg-accent focus:px-4 focus:py-2 focus:font-semibold focus:text-accent-foreground"
      >
          ...
          <div className="hidden items-center gap-9 lg:flex">
            {menuLinks.map((link) => (
              <a
                ...
                font-medium text-muted-foreground transition-colors duration-300 hover:text-foreground"
              >
                {link.label}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-[9990] flex min-h-screen flex-col bg-background/98 backdrop-blur-2xl lg:hidden"
          >
            {/* ================= ШАПКА ================= */}
            ...
            <div className="flex items-center justify-between px-6 py-6">
              ...
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-xl border border-border text-foreground transition-colors hover:border-accent/50 hover:text-accent"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="flex flex-1 flex-col justify-center gap-3 overflow-y-auto px-6 py-8">
              {menuLinks.map((link, index) => (
                <motion.a
                  ...
                  className="font-display text-4xl font-medium uppercase tracking-wide text-foreground/90 transition-colors hover:text-accent sm:text-5xl"
                >
                  {link.label}
                </motion.a>
              ))}
            ...
            <div className="flex flex-col gap-3 px-6 pb-8">
              ...
              <button
                ...
                className="btn-glow flex items-center justify-center gap-2 rounded-2xl bg-accent py-4 font-semibold
              ...
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => setOpen(false)}
                className="flex items-center justify-center gap-2 rounded-2xl border border-border py-4 font-semibold text-foreground transition-colors hover:border-accent/50 hover:text-accent"
              >
                <MessageCircle className="h-5 w-5" />
                WhatsApp
              </a>
