...
export function Navbar() {
   ...
   *
   * Для якорей (#hero, #inventory, #credit и т.д.)
  ...
  const handleNavigation = (
     ...
     * чтобы заголовок раздела не оказался под Navbar.
     */
  ...
  return (
    <>
      ...
      <a
        ...
        className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-[100001] focus:rounded-xl focus:bg-accent focus:px-4 focus:py-2 focus:font-semibold focus:text-accent-foreground"
      >
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-expanded={open}
              className="relative z-[99999] flex h-12 w-12 touch-manipulation items-center justify-center rounded-xl border border-border text-foreground transition-colors hover:border-accent/50 lg:hidden"
            >
        ...
        createPortal(
          <AnimatePresence>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[100002] flex min-h-screen items-center justify-center bg-black/75 p-6 backdrop-blur-md"
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                className="fixed inset-0 z-[100000] flex min-h-screen flex-col bg-black lg:hidden"
                style={{
                  pointerEvents: 'auto',
              ...
              >
                ...
                <div className="flex shrink-0 items-center justify-between px-6 py-6">
                  <span className="font-display text-lg font-semibold tracking-[0.22em] text-white">
                    БАЗА
                  </span>
                  ...
                  <button
                    ...
                    className="relative z-[100001] flex h-12 w-12 touch-manipulation items-center justify-center rounded-xl border border-white/20 text-white transition-colors hover:border-accent/60 hover:text-accent"
                  >
                    <X className="h-6 w-6" />
                  </button>
                </div>
                ...
                <nav className="flex flex-1 flex-col justify-center gap-1 overflow-y-auto px-6">
                  {navLinks.map((link, i) => (
                    <motion.a
                      ...
                      className="block touch-manipulation py-3 font-display text-3xl font-medium uppercase tracking-wide text-white transition-colors hover:text-accent"
                    >
                      {link.label}
                    </motion.a>
                  ))}
                ...
                <div className="shrink-0 space-y-3 px-6 pb-8">
                  {/* ПОЗВОНИТЬ */}
                  ...
                  <button
                    ...
                    className="btn-glow flex w-full items-center justify-center gap-2 rounded-2xl bg-accent py-4 font-semibold text-accent-foreground"
                  >
                    <Phone className="h-5 w-5" />
                    Позвонить
                  </button>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/20 py-4 font-semibold text-white transition-colors hover:border-accent/60 hover:text-accent"
                  >
                    <MessageCircle className="h-5 w-5" />
                    WhatsApp
                  </a>
